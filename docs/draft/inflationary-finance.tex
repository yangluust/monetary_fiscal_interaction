...
The problem is that the future cost of this reserve finance does not feed back into the government's repayment-default comparison. The period-$1$ and period-$2$ central-bank constraints imply
\[
M_1 = Q^B_1 B^{CB}_1 + (1+i^R_0)M_0,
\]
\[
T^{CB,R}_2 = B^{CB}-(1+i^R_1)M_1,
\qquad
T^{CB,D}_2 = -(1+i^R_1)M_1.
\]
Hence
...
T^{CB,R}_2-T^{CB,D}_2 = B^{CB},
\]
so the reserve liability $(1+i^R_1)M_1$ enters repayment and default symmetrically and cancels out of the default condition:
\[
P_2(Y+\phi)+T^{CB,R}_2-B<P_2\kappa+T^{CB,D}_2
\]
\[
\Longleftrightarrow
\qquad
P_2(Y+\phi)+B^{CB}-B<P_2\kappa.
\]
As a result, default probabilities depend only on
\[
D\!\left(\kappa_i + \frac{B-B^{CB}}{P_2} - \phi_i\right),
\]
not directly on $M_0$ or $M_1$. Since reserves are voluntarily held, banks face no explicit reserve-holding friction, and the central bank can run negative net worth, sufficiently large reserve issuance can in principle eliminate the need for risky sovereign borrowing. 

A first minimal way to discipline this margin is to introduce a downward-sloping reserve demand curve, motivated by the idea that reserves provide liquidity services to banks, help absorb payment shocks, and satisfy reserve or regulatory needs \citep{ChenKourentzesVeyrune2023}. A simple way to formalize this is to let banks derive utility not only from consumption but also from real reserve holdings:
\[
...
A second minimal way to discipline reserve issuance is to impose a central-bank solvency or fiscal-support constraint. Following the distinction emphasized by \citet{DelNegroSims2015}, \citet{HallReis2015}, and \citet{Reis2017QE}, fiscal backing of the price level is not the same as fiscal support for the central bank. Even if monetary policy determines inflation in equilibrium, a sufficiently impaired central-bank balance sheet may require recapitalization or else force the central bank to tolerate higher inflation or exploding reserve liabilities. A reduced-form analogue in the present finite-horizon environment is to impose a lower bound on period-$2$ remittances,
\[
T^{CB,R}_2 \ge -\bar{\Xi},
\qquad
T^{CB,D}_2 \ge -\bar{\Xi},
