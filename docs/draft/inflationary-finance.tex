...
A simple payment-services microfoundation can rationalize this reduced form. Let $Z_t$ denote aggregate payment services in period $t$, and suppose one unit of non-bank consumption requires one unit of payment services, so that in equilibrium
\[
Z_t = (1-\mu) C^N_t.
\]
Suppose also that banks must hold reserves to settle these payments. If the reserve requirement is
\[
\frac{M_t}{P_t} \ge \theta Z_t,
\]
then, at the per-bank level,
\[
m_t \ge \theta P_t \frac{1-\mu}{\mu} C^N_t.
\]
Let $\lambda_t$ denote the multiplier on this reserve requirement in the bank's problem. Then the bank's reserve Euler equation takes the form
\[
\frac{u'(C^A_t)}{P_t}
=
\lambda_t
+ \beta(1+i^R_t)\,\mathbb{E}_t\!\left[\frac{u'(C^A_{t+1})}{P_{t+1}}\right].
\]
The term $\lambda_t$ is the shadow value of reserves coming from their role in settlement.

To see why this shadow value is decreasing in reserve holdings, consider a static payment-services subproblem. Let $s_t$ denote the real fee per unit of payment services paid by non-banks to banks. Taking $s_t$ as given, non-banks choose $C^N_t$ and $Z_t$ to solve
\[
\max_{C^N_t,Z_t}\; (1-\mu)u(C^N_t)-s_t Z_t
\]
subject to
\[
Z_t \ge (1-\mu) C^N_t.
\]
At the optimum the constraint binds, implying
\[
s_t = u'(C^N_t).
\]
...
s_t(x_t)
=
u'\!\left(\frac{\mu}{\theta(1-\mu)}x_t\right).
\]
\noindent To see how this enters the bank's intertemporal optimality condition, let
\[
\nu_t \equiv \frac{u'(C^A_t)}{P_t},
\qquad
\omega_t \equiv \beta(1+i^R_t)\,\mathbb{E}_t\!\left[\frac{u'(C^A_{t+1})}{P_{t+1}}\right]
\]
denote, respectively, the marginal utility value of one nominal unit in period $t$ and the continuation value of carrying one more unit of reserves into period $t+1$. At the per-bank level, let
\[
z_t \equiv \frac{Z_t}{\mu} = \frac{1-\mu}{\mu}C^N_t
\]
denote payment services supplied by one bank, so the reserve requirement is
\[
m_t \ge \theta P_t z_t.
\]
Taking $s_t$, $\nu_t$, and $\omega_t$ as given, the bank's static payment-services subproblem is
\[
\max_{m_t,z_t}\;
\nu_t P_t s_t z_t - \nu_t m_t + \omega_t m_t
\]
subject to $m_t \ge \theta P_t z_t$. With multiplier $\lambda_t$ on the reserve constraint, the first-order conditions are
\[
\nu_t P_t s_t - \lambda_t \theta P_t = 0,
\qquad
-\nu_t + \omega_t + \lambda_t = 0.
\]
Hence,
\[
\lambda_t = \nu_t \frac{s_t}{\theta},
\qquad
\nu_t = \lambda_t + \omega_t.
\]
Substituting back the definitions of $\nu_t$ and $\omega_t$ yields
\[
\frac{u'(C^A_t)}{P_t}
=
\lambda_t
+ \beta(1+i^R_t)\,\mathbb{E}_t\!\left[\frac{u'(C^A_{t+1})}{P_{t+1}}\right],
\]
which is exactly the reserve Euler equation in the microfounded model.

It is then useful to normalize the shadow value of reserves by the bank's marginal utility of nominal wealth. Define
\[
\tilde \lambda_t(x_t)\equiv \frac{\lambda_t(x_t)}{\nu_t}.
\]
Using the first-order condition above,
\[
\tilde \lambda_t(x_t)
=
\frac{s_t(x_t)}{\theta}
...
<0,
\]
because $u''(\cdot)<0$. Thus the normalized shadow value of reserves declines with reserve holdings. The reduced-form specification is obtained by defining
\[
\chi_t \ell'(x_t)\equiv u'(C^A_t)\tilde \lambda_t(x_t).
\]
Under this mapping,
\[
\frac{\chi_t \ell'\!\left(m_t/P_t\right)}{P_t}
...
\]
so the convenience-yield term in the reduced-form reserve Euler equation is exactly the nominal counterpart of the shadow value generated by the payment-services microfoundation. The concavity condition $\ell''(\cdot)<0$ is therefore the reduced-form representation of diminishing marginal utility of non-bank consumption.
