...
A simple payment-services microfoundation can rationalize this reduced form. Let $z^N_t$ denote payment services used by each non-bank in period $t$, and suppose one unit of non-bank consumption requires one unit of payment services, so that in equilibrium
\[
z^N_t = C^N_t.
\]
Aggregate payment services are $Z_t=(1-\mu)z^N_t$. At the per-bank level, let
\[
...
m_t \ge \theta P_t z_t.
\]
Let $s_t$ denote the real fee per unit of payment services paid by non-banks to banks. The fee applies only in periods $t=0,1$, because these are the periods in which reserves are held and used for settlement. Thus the non-bank problem can be written intertemporally, in per-non-bank terms, as
\[
...
\]
and the payment-services requirements
\[
z^N_t \ge C^N_t,
\qquad t=0,1.
\]
...
Let $\nu^N_t$ denote the current-value non-bank marginal utility of one nominal unit in period $t$, and let $\xi_t$ denote the current-value multiplier on the payment-services requirement. The first-order conditions for $C^N_t$ and $z^N_t$ imply
\[
u'(C^N_t)
=
\nu^N_t P_t + \xi_t,
...
\qquad t=0,1.
\]
Thus, for a given non-bank marginal value of wealth $\nu^N_t P_t$, the service fee is decreasing in $C^N_t$ because $u''(\cdot)<0$.
...
\]
Since the payment-services requirement binds, $z^N_t=C^N_t$.
...
\tilde \lambda_t(x_t)
\equiv
\frac{\lambda_t(x_t)}{u'(C^A_t)/P_t}.
\]
Using the payment-service first-order condition above,
\[
\tilde \lambda_t(x_t)
=
\frac{s_t(x_t)}{\theta}
...
<0,
\]
holding $\nu^N_t P_t$ fixed and using $u''(\cdot)<0$. Thus the normalized shadow value of reserves declines with reserve holdings along the non-bank inverse demand curve. The reduced-form specification is obtained by defining
\[
