...
\]
Now let $\tilde \lambda_t(x_t)\equiv P_t\lambda_t(x_t)$ denote the real shadow value of reserves, where $\lambda_t$ is the nominal multiplier that appears in the bank's Euler equation above. Since one extra unit of $x_t$ supports only $1/\theta$ extra units of payment services, the real shadow value is
\[
\tilde \lambda_t(x_t)
=
\frac{1}{\theta}s_t(x_t)
...
<0,
\]
because $u''(\cdot)<0$. Thus the shadow value of reserves declines with reserve holdings. The reduced-form specification is obtained by defining
\[
\chi_t \ell'(x_t)\equiv \tilde \lambda_t(x_t).
\]
Under this mapping,
\[
\frac{\chi_t \ell'\!\left(m_t/P_t\right)}{P_t}
